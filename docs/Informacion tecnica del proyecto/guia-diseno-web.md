...
| `AbilityPlaque` | Característica con el **modificador como cifra principal**, la puntuación rotulada debajo (`Punt. 20`) y la salvación competente nombrada (`SALV` con escudo) | Reserva el alto de la marca aunque no haya salvación, para que las seis alineen. Es **de la ficha**: marca *si* hay competencia, porque el número lo calcula el motor y se lee en Salvaciones. El perfil de criatura no la usa — una criatura trae la salvación ya impresa y ahí se muestra el valor (ver `creatureProfileBody`) |
...
| `CreatureActionRow` | Una acción de criatura: nombre, bonificador con signo en Georgia dorado, daño con su tipo, alcance y descripción | Toma **primitivos** y no un `CreatureAction`. Hoy la usa **solo la ficha** para los compañeros invocados (vía `ResolvedCreatureAction`): el perfil del Bestiario dejó de usarla cuando el daño pasó a placas rotuladas |
| `creatureProfileBody(…)` | El perfil de una criatura del bloque de cifras para abajo: banda de cifras (CA, PG con sus dados, iniciativa, desafío, percepción pasiva), banda de las seis características con la salvación adentro, líneas como lista de definición con las defensas en chips, rasgos y acciones agrupadas por tipo, con acierto/daño/alcance en placas rotuladas | Devuelve `List<Widget>`, no un `Column`: cada llamador lo mete en su propio `ListView` con su encabezado (el nombre y el tipo **no** van adentro). Lo leen el Bestiario y la columna «Del turno» de Combate. **Dos ejes distintos, y conviene no confundirlos**: `dense: true` decide *contenido* —saca CA, PG e iniciativa, que en Combate los lleva el encuentro y no el catálogo— mientras que la *forma* la decide un `LayoutBuilder` con el ancho medido (`_profileWideWidth`, 520): por debajo se apila todo, que es lo que necesitan la columna «Del turno» de 300 px y el Bestiario en un teléfono. La prosa va topeada a `_profileProseWidth` (620) porque el panel del Bestiario mide unos 855 px |
