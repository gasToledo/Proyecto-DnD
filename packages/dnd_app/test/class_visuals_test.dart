...
void main() {
  test('parseo del color de acento y fallback', () {
    ...
    const klass = CharacterClass(
      id: 'x', name: 'X', source: ContentSource.homebrew, hitDie: 8,
      accentColor: '#B0413E', iconId: 'shield',
    );
    ...
    const bad = CharacterClass(
      id: 'y', name: 'Y', source: ContentSource.homebrew, hitDie: 8,
      accentColor: 'nope', iconId: 'inexistente',
    );
  ...
  test('cada clase oficial declara acento válido e ícono reconocido', () async {
    ...
    for (final klass in repo.classes.values) {
      expect(klass.accentColor, isNotNull, reason: '${klass.id} sin accentColor');
      expect(classAccent(klass, fb), isNot(fb),
          reason: '${klass.id}: accentColor no parsea');
      expect(classIcon(klass), isNot(Icons.person_outline),
