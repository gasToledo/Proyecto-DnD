...
- [x] 1.6 Verificar en un pull request de prueba que el resultado de los
...
- [x] 2.1 Configurar protección de rama sobre `main`: requerir pull request,
      requerir que los jobs de CI pasen, sin _force-push_ (ajuste en GitHub
      _Settings → Branches_, ver `docs/despliegue.md`; no ejecutable desde
      esta sesión, requiere acceso de administrador al repositorio)
- [x] 2.2 Configurar la misma protección sobre `webapp` mientras siga activa
- [x] 2.3 Confirmar quién tiene permiso de push directo/merge sobre ambas
      ramas y ajustarlo si hace falta (ver design.md, D5: es el único
      control de acceso al despliegue)

## 3. Runner autoalojado

- [x] 3.1 Instalar el runner de GitHub Actions como servicio en el host que
      ...
      sesión)
- [x] 3.2 Confirmar que el usuario del sistema bajo el que corre el runner
      tiene permiso para invocar `docker compose` en ese host
- [x] 3.3 Colocar `.env`, `cloudflared/config.yml` y `cloudflared/creds.json`
...
- [x] 3.4 Verificar con un workflow mínimo de prueba que el runner queda
...
- [x] 4.6 Probar el filtro de rutas: un push que solo toca `docs/**` no
      ...
      esta sesión)
- [x] 4.7 Probar el filtro de rama: un push a una rama distinta de
...
- [x] 6.1 Primer despliegue real disparado por `cd.yml` contra el host de
      producción
- [x] 6.2 Con ese despliegue real, retomar las tareas bloqueadas de
      `migrate-to-self-hosted-webapp` que necesitaban un host real: 10.7,
      10.8, 10.9, 10.10, 10.12 y 11.1
- [x] 6.3 Confirmar que un push que dispara CD también pasó CI antes (por la
