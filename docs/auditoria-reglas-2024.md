...
| Dotes | 17 | 87 |
| Conjuros | 388 (PHB, cap. 7) | 389 |

El catálogo es más grande que el SRD en todos los bloques, con contenido del
PHB 2024 y, desde Q4/Q5, de *Forge of the Artificer* (2025). El único conjuro
que excede el capítulo 7 del PHB es Sirviente Homúnculo, de ese libro. Sin la
etiqueta correcta, todo esto se confundiría bajo la misma atribución.

## Pendientes abiertos al cierre de esta tanda

Ordenados por costo, del más barato al más caro:

1. **Nomenclatura**: el catálogo usa traducciones propias donde el SRD tiene
   nombre oficial (Aprendiz de Mucho, Sentir el Peligro, Truco Potente). Esta
   tanda solo alineó los rasgos que tocó.
2. **Elección de característica en dotes** (+1 a X o Y): hoy está fija en el
   dato y anotada en el texto. Requiere efecto nuevo, campo persistido,
   migración y dos superficies de UI.
3. **Estilo de Combate de Paladín y Explorador**: confirmado contra el PHB que
...
4. **Agotamiento e Inspiración Heroica**: ambos existen como dato pero no
   afectan ningún cálculo. En 2024 el agotamiento da −2 por nivel a las pruebas
   de d20 y −5 pies de velocidad.
5. **Precio y peso del equipo**: no existen como campo en `Weapon` ni en
   ...
   carga.
6. **Invocaciones Sobrenaturales**: el sistema no existe en el motor, así que
   los cuatro pactos del Brujo solo figuran como texto.
7. **Objetos mágicos temporales y compañeros con estadísticas propias**: no
